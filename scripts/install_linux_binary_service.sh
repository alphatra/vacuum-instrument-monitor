...
UNIT_SRC="$APP_DIR/systemd/vacuum-monitor-collector-binary@.service"
UNIT_DST="/etc/systemd/system/vacuum-monitor-collector-binary@.service"
...
if [ ! -x "$BINARY_PATH" ]; then
  if [ "$BUILD_IF_MISSING" = "true" ]; then
    "$APP_DIR/scripts/build_linux_binary.sh" --onefile --install "$APP_DIR/bin"
  else
...
systemctl enable "vacuum-monitor-collector-binary@${INSTANCE}.service"
systemctl restart "vacuum-monitor-collector-binary@${INSTANCE}.service"

echo "Installed: vacuum-monitor-collector-binary@${INSTANCE}.service"
